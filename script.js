...
class Canvas {
    ...
    loadAudio() {
        this.audio.src = "assets/music.mp3"; // Укажите путь к вашему файлу музыки
        this.audio.loop = true; // Включаем зацикливание музыки
        this.audio.volume = 1; // Устанавливаем громкость (от 0 до 1)
    ...
    offInit() {
        ...
        // Запускаем музыку и показываем картинку после рассыпания
        this.offHearts.forEach(heart => heart.breakHeart());
        this.loadAudio(); // Запуск музыки
        this.showImage(); // Отображение картинки
    }
    showImage() {
        const img = new Image();
        img.src = 'assets/image.png'; // Укажите путь к вашему изображению
        img.onload = () => {
            this.ctx.drawImage(img, this.width / 2 - img.width / 2, this.height / 2 - img.height / 2); // Центрируем изображение
        };
    }
    render() {
...
class Heart {
    ...
    init(x, y, r, i) {
        ...
        this.i = i * 0.5 + 200;
        this.l = this.i;
        this.c = Tool.randomColorHSL(Tool.randomNumber(240, 270), 80, 60);
    ...
    breakHeart() {
        this.hasBroken = true; // Устанавливаем флаг, что сердце рассыпалось
        // Здесь можно добавить логику для анимации рассыпания
    }
    offRender(i) {
        this.draw();
    }
